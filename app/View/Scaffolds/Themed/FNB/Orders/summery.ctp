...
<section class="dish-ingredient-sec">
	...
	<section class="oder-sumery-section clearfix" id="billing">
		...
		<ul class="bank-account  billing-section">
			...
			<li>
				<?php echo $this->Form->input('User.city',array('options'=>array(),'div'=>false,'label'=>false,'class'=>'kitc-infield state-fld','empty'=>'Select City'));
 	...
 	<section class="oder-sumery-section clearfix hide" id="summery">
	    <section class="order-summary" id="orderSummery">
			    ...
			    <div class="items-info">
					...
					?>	
						...
						<?php
							...
							$total += $price;
							$totalTax = round((($kitchenForOrderSummery['Kitchen']['sales_tax'] / 100 ) * $total),2);
							$mainTotal = $total+$totalTax;
			...
			<div class="total-payment">
				...
				<span class="fright"><?php echo "$".$total; ?></span>
			</div>
		</section>
	         ...
	         <div class="dinning-options">
			    <span>
					...
					<b class="serving-timer">
						<?php
							$searchData =  $this->Session->read('search_data.t');
				...
				<span>
					<b>LOCATION </b><br>
					 ...
					 <b class="location-icon">
					 	<?php 		echo $this->common->distance( $kitchenForOrderSummery['Kitchen']['lat'],  $kitchenForOrderSummery['Kitchen']['lng'], $userDetails['User']['lat'], $userDetails['User']['lng'], "M") . " miles"; ?>
					 </b>
					 <?php } ?>
				  		...
				  		<div class="popupContainer">
							<ul class="cart-info">
								...
								<li>
									...
									echo $this->Form->checkbox('PaymentMethod.id.'.$pValue['id'], array('hiddenField'=>false,'class'=>'checkbox', 'div'=>'true','checked' => $checkedPay)); ?>
									<figure class="card-img"><?php echo $this->Html->image($pValue['type'].'.png'); ?></figure>
									<span class="card-number"><?php echo $pValue['card_no']; ?></span>
								</li>	
								<li>
				  		...
				  		<div class="discussion-container">
							<ul>
								<li>
									...
									<?php echo $this->Form->input('PaymentMethod.card_no', array('label'=>false,'div'=>false,'class'=>'disc-field numeric','autocomplete'=>"off")); ?>
								</li>
								<li>
									<label> Card holder name</label>
									<?php echo $this->Form->input('PaymentMethod.card_name', array('label'=>false,'div'=>false,'class'=>'disc-field','autocomplete'=>"off")); ?>
								</li>
								<br/>
								<li>
									<div class="width50percent fleft">
										...
										echo $this->Form->input('PaymentMethod.exp_month', array('class'=>'community-select','empty'=>'Exp Month','label'=>false,'options'=>$expMonth)); ?>
									</div>
									<div class="width50percent fright">
										...
										echo $this->Form->input('PaymentMethod.exp_year', array('class'=>'community-select','empty'=>'Exp Year','label'=>false,'options'=>$expYear)); ?>
									</div>
								</li>
								<br/>
								<li>
									<div class="width50percent fleft">
										<label>Cvv No.</label>
										<?php echo $this->Form->input('PaymentMethod.cvv_no', array('label'=>false,'div'=>false,'class'=>'disc-field numeric','autocomplete'=>"off")); ?>
									</div>
									<div class="width50percent fright">
...
function setCityOption(id){
		$.ajax({
				...
				'success': function(output) {
					$('#UserCity').html(output);
					<?php if(isset($this->request->data['User']['city']) && !empty($this->request->data['User']['city'])){  ?>
						$("#UserCity").val('<?php echo $this->request->data['User']['city']; ?>');
					<?php 	} ?>
				}
			});
}
