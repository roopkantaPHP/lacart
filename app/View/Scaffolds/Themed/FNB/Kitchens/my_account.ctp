		  ...
		  <?php echo $this->Form->create('User', array('type' => 'file')); ?>
			<main class="discussion-container">
				<ul>
						...
						<div class="uploaded-img">
							...
							$imgName = 'img1.png';
							 if(isset($this->request->data['User']['image']) && !empty($this->request->data['User']['image'])){
								if(FILE_EXISTS(PROFILE_IMAGE_URL.$this->request->data['User']['image'])){
									$imgName = PROFILE_IMAGE_FOLDER.$this->request->data['User']['image'];
								}
							}
					...
					<li class="textbox">
						<?php echo $this->Form->textarea('description',array('class'=>'community-select'));  ?>
						 <span class="char-info"><?php echo "200 characters"; ?></span>
