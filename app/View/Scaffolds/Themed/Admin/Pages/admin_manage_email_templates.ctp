...
<div class="row">
	...
	<div class="floatright"><?php echo $this->Html->link('<span>Add Email Template</span>', array('controller' => 'pages', 'action' => 'add_email_template','admin' => true), array('class' => 'black_btn', 'escape' => false)) ?></div>
</div>
					...
					<td align="center">
						<?php 
							echo $this->Form->postLink(__('Delete'), array(
								'controller' => 'pages', 'admin' => true,
								'action' => 'manage_email_templates', $row['id']), null,
								__('Are you sure you want to delete # %s?', $row['subject'])
							);
						?>
						<?php echo $this->Html->link('<img src="'.SITE_URL.'/admin_images/edit_icon.gif" title="Edit Newsletter" border="0"/>', array('controller'=>'pages', 'action'=>'edit_email_template', $row['id']), array('escape' => false)) ?>
					</td>
				</tr> 
			<?php
					$i++; 
