...
class HybridauthComponent extends Component {
	...
	protected function normalizeSocialProfile($provider){
		...
		$socialProfile['User']['facebook_id'] = $incomingProfile['identifier'];
		else if($provider=='Google')
		$socialProfile['User']['google_id'] = $incomingProfile['identifier'];
		if(isset($incomingProfile['email']) && !empty($incomingProfile['email']))
		  $socialProfile['User']['email'] = $incomingProfile['email'];
		else
        {
            if($provider=='Facebook')
                $socialProfile['User']['email'] = $incomingProfile['firstName']."_".$incomingProfile['lastName'].'@facebook.com';
            else if($provider=='Google')
                $socialProfile['User']['email'] = $incomingProfile['firstName']."_".$incomingProfile['lastName'].'@google.com';;
        }

        $socialProfile['User']['username'] = $incomingProfile['displayName'];
		$socialProfile['User']['name'] = $incomingProfile['firstName']." ".$incomingProfile['lastName'];
		$socialProfile['User']['link'] = $incomingProfile['profileURL'];
		$socialProfile['User']['image'] = $incomingProfile['photoURL'];
		$socialProfile['User']['created'] = date('Y-m-d h:i:s');
		$socialProfile['User']['modified'] = date('Y-m-d h:i:s');
		$socialProfile['User']['group_id'] = NORMAL_USER;
		$socialProfile['User']['phone'] = (!empty($incomingProfile['phone'])) ? $incomingProfile['phone'] : '';
								
			
		// twitter does not provide email so we need to build someting
		/*if($provider == 'Twitter'){
			$names = explode(' ', $socialProfile['SocialProfile']['first_name']);
			$socialProfile['User']['name'] = $names[0]." ".(count($names)>1 ? end($names) : '');
			$socialProfile['User']['username'] = $socialProfile['User']['first_name'] .'_'. $socialProfile['SocialProfile']['last_name'];
			$socialProfile['User']['email'] = $socialProfile['User']['username'] .'@Twitter.com';
		}*/
